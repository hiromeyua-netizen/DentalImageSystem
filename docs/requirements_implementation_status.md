...
Traceability: `project_requirements.txt` (SOW) vs current codebase (`app/` PyQt6+QML + `dental_imaging/` + `firmware/`).

## Implemented (matches SOW intent)

| Area | Requirement | Where / notes |
|------|-------------|----------------|
| Camera | Basler via pypylon, USB, connect/disconnect | `dental_imaging/hardware/camera/basler_camera.py`; `app/camera_service.py` power / auto-connect |
...
| Boot splash | SOW: logo splash; `default_config.json` / `dental_imaging` settings mention splash; **`app/main.py` does not show a splash window** | Add Qt splash or QML splash using `resources/` if still required |
